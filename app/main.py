...
def load_data() -> None:
    ...
    # Load images manifest (person_images.json generated by scrape_images.py)
    images_manifest_path = base_path / "data" / "processed" / "person_images.json"
    if images_manifest_path.exists():
        with open(images_manifest_path, 'r') as f:
            DATA['images'] = json.load(f)
    else:
        # Legacy path fallback
        legacy = base_path / "app" / "static" / "images" / "people" / "images_manifest.json"
        if legacy.exists():
